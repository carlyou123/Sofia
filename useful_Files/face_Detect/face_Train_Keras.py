...
class Model:
    ...
    # 训练模型
    def train(self, dataset, batch_size=56, nb_epoch=15, data_augmentation=True):
        ...
        # 图像预处理
            ...
            self.model.fit_generator(datagen.flow(dataset.train_images, dataset.train_labels,batch_size=batch_size), steps_per_epoch = 10,
                                     epochs=nb_epoch
    ...
    def evaluate(self, dataset):
        score = self.model.evaluate(dataset.test_images,
                                    dataset.test_labels)  # evaluate返回的结果是list，两个元素分别是test loss和test accuracy
...
if __name__ == '__main__':
    ...
    dataset = Dataset('.\\images_Data_Training\\')
    dataset.load()
    # 训练模型
    model = Model()
